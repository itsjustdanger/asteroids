(function(){
	...
	Game.prototype.addAsteroids = function() {
		for (var i = 0; i < this.NUM_ASTEROIDS; i++) {
			this.asteroids.push(new Asteroids.Asteroid(this.randomPosition(), 40, this, false));
		};
	};
	...
	Game.prototype.draw = function(ctx, img, bg_y) {
		ctx.clearRect(0, 0, this.DIM_X, this.DIM_Y);
		ctx.drawImage(img, 0, bg_y);
		ctx.drawImage(img, 0, (bg_y - this.DIM_Y));
	...
	Game.prototype.removeAsteroid = function(asteroid){
		for (var i = 0; i < this.asteroids.length; i++) {
			if (this.asteroids[i].pos === asteroid.pos) {
				if (this.asteroids[i].frag === false){
					this.asteroids[i].fragment();
				}
